...
from agent.freellmapi_client import _extract_json
...
def test_extract_json_handles_bare_and_wrapped_replies():
    """Regression: real chat models wrap JSON in prose / ```json fences``` even
    when told not to. _extract_json must recover the object so a valid live
    response is not silently discarded to the heuristic (see freellmapi_client)."""
    bare = _extract_json('{"risk": 0.8, "label": "threat"}')
    assert bare["risk"] == 0.8

    fenced = _extract_json('Here is the result:\n```json\n{"risk": 0.2, "label": "low"}\n```\nDone.')
    assert fenced["label"] == "low"
...
def test_adjudicator_fallback_preserves_frame_index():
    """Regression test: the no-provider fallback path used to construct
    RiskEvent without `frame_index`, raising TypeError instead of degrading
    gracefully -- the exact failure mode the safety model promises never
    happens. Locks in the fix in agent/adjudicator.py.
    """
    adj = Adjudicator(provider=None)
    decision = adj.decide(_detections(items=[]))
    assert decision.event.frame_index == 0

    detections = ValidatedDetections(frame_index=42, items=[], dropped=0)
    decision = adj.decide(detections)
    assert decision.event.frame_index == 42
